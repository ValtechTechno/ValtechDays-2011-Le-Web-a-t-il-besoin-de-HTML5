...
SlideShow.prototype = {
 ...
 go: function(slideId, update) {
   this.current = slideId;
   this._update(update != undefined ? update : true);
 },
 ...
 handleKeys: function(e) {
   ...
   if (/^(input|textarea)$/i.test(e.target.nodeName) || e.target.isContentEditable) {
	...
	if (keyLock) return; // If keylock flag, return !
...
var linkEls = query('link.theme');
var stylesheetPath = localStorage['theme'] || 'css/valtech.css';
